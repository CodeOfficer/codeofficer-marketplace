...
function log(message) {
  ...
  try {
    appendFileSync("/tmp/claude-hook-check-git-commit.log", `[${timestamp}] [marketplace] ${message}\n`);
  } catch {}
}
...
async function main() {
  try {
    ...
    // Match all -m "message" or -m 'message' flags - use DOTALL (s flag) to match across newlines
    ...
    log(`Extracted ${messages.length} message(s) from command`);
    log(`Full command: ${command.substring(0, 500)}`);

    // Check if any message contains the context marker
    const hasContext = messages.some(msg => {
      const found = new RegExp(CONTEXT_MARKER, "m").test(msg);
      log(`Message check: "${msg.substring(0, 100)}..." has context: ${found}`);
      return found;
    });
