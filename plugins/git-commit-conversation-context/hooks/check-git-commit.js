 ...
 *   CLAUDE_PLUGIN_ROOT - Path to plugin root (automatically set by Claude Code)
 *
 * Exit codes:
...
async function main() {
  try {
    ...
    if (isGitCommit) {
      ...
      // Check if the git-commit-conversation-context skill is installed
      // Use CLAUDE_PLUGIN_ROOT if available (plugin context), fallback to cwd-based check (project context)
